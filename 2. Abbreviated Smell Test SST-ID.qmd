...
# Save the rankings
write.csv(rank_comp, "SST_ID_rankings.csv", row.names = FALSE)
```
...
optionLabels <- as.matrix(read.csv("SST_ID_option.csv",header = F, check.names=FALSE))
...
saveRDS(df_long_M, "df_sex_D_M.rds")
```
...
saveRDS(df_long_F, "df_sex_D_F.rds")
```
...
saveRDS(df_fine, "df_fine_D.rds")
saveRDS(df_long_Age, "df_age_D.rds")
```
