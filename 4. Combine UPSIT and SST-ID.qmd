...
write.csv(df_shared, "Generated files/df_shared.csv", row.names = FALSE)
```
...
# zenodo data
df_O_sub <- df_O[, c("cohort", "ID", "group", "diagnosis", "sex", "age_cat", "UPSIT", "olfaction", sub_UPSIT)]
# original data
...
# zenodo data
df_P_sub <- df_P[, c("cohort", "ID", "group", "diagnosis", "sex", "age_cat", "UPSIT", "olfaction", sub_UPSIT)]
# original data
...
# zenodo data
df_D_sub <- df_D[, c("cohort", "ID", "EVENT_ID", "group", "diagnosis", "sex", "age_cat", "SST.ID", "olfaction", sub_SST_ID)]
# original data
...
temp <- (roc_1$sensitivities + roc_1$specificities)/2
print("PD/DLB vs HC in the combined cohort")
print(paste0(group_name[2], " vs ", group_name[1], ": cut = ", cut_1, 
             "; sensitivity = ", round(roc_1$sensitivities[roc_1$thresholds == cut_1],2),
             "; specificity = ", round(roc_1$specificities[roc_1$thresholds == cut_1],2),
             "; C-stat = ", round(temp[roc_1$thresholds == cut_1], 2)))

roc_1 <- roc(df_bl$group, df_bl$score, levels=c(group_name[c(3, 2)]), na.rm=TRUE, ci=TRUE) 
temp <- (roc_1$sensitivities + roc_1$specificities)/2
print("PD/DLB vs MSA/PSP in the combined cohort")
print(paste0(group_name[2], " vs ", group_name[3], ": cut = ", cut_2, 
             "; sensitivity = ", round(roc_1$sensitivities[roc_1$thresholds == cut_2],2),
             "; specificity = ", round(roc_1$specificities[roc_1$thresholds == cut_2],2),
             "; C-stat = ", round(temp[roc_1$thresholds == cut_2], 2)))
...
df_D_tbl1 <- df_bl %>% 
  ...
  droplevels() %>% 
  select(group, 
         sex, score_old, score, olfaction) 
names(df_D_tbl1)[c(2:ncol(df_D_tbl1))] <- c("Sex",  
...
df_O_tbl1 <- df_bl %>% 
  ...
  select(group, 
         sex, score_old, score, olfaction) 
names(df_O_tbl1)[c(2:ncol(df_O_tbl1))] <- c("Sex", 
...
df_P_tbl1 <- df_bl %>% 
  ...
  select(group, 
         sex, score_old, score, olfaction) 
names(df_P_tbl1)[c(2:ncol(df_P_tbl1))] <- c("Sex", 
                                            "UPSIT", "7 scents", "Olfaction")
