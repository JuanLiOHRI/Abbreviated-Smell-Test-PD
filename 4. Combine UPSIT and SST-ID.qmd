...
ranks_SST_ID <- read.csv("SST_ID_rankings.csv", header = T)
ranks_UPSIT <- read.csv("UPSIT_rankings.csv", header = T)
```
...
write.csv(df_shared, "df_shared.csv", row.names = FALSE)
```
...
df_sex_D <- readRDS("df_sex_D.rds") %>% mutate(cohort = "DeNoPa") %>% 
  filter(item %in% df_shared$scent) 
df_F_D <- readRDS("df_sex_D_F.rds") %>% mutate(cohort = "DeNoPa") %>% 
  filter(item %in% df_shared$scent) 
df_M_D <- readRDS("df_sex_D_M.rds") %>% mutate(cohort = "DeNoPa") %>% 
  filter(item %in% df_shared$scent) 
df_sex_O <- readRDS("df_sex_O.rds") %>% mutate(cohort = "Ottawa Trial") %>% 
  filter(item %in% df_shared$scent) 
df_F_O <- readRDS("df_sex_O_F.rds") %>% mutate(cohort = "Ottawa Trial") %>% 
  filter(item %in% df_shared$scent) 
df_M_O <- readRDS("df_sex_O_M.rds") %>% mutate(cohort = "Ottawa Trial") %>% 
  filter(item %in% df_shared$scent) 
df_sex_P <- readRDS("df_sex_P.rds") %>% mutate(cohort = "PROBE") %>% 
  filter(item %in% df_shared$scent) 
df_F_P <- readRDS("df_sex_P_F.rds") %>% mutate(cohort = "PROBE") %>% 
  filter(item %in% df_shared$scent) 
df_M_P <- readRDS("df_sex_P_M.rds") %>% mutate(cohort = "PROBE") %>% 
  filter(item %in% df_shared$scent) 
...
df_fine_D <- readRDS("df_fine_D.rds") %>% filter() %>% mutate(cohort = "DeNoPa") %>% 
  filter(item %in% df_shared$scent) 
df_age_D <- readRDS("df_age_D.rds") %>% mutate(cohort = "DeNoPa") %>% 
  filter(item %in% df_shared$scent) 
df_fine_O <- readRDS("df_fine_O.rds") %>% filter() %>% mutate(cohort = "Ottawa Trial") %>% 
  filter(item %in% df_shared$scent)
df_age_O <- readRDS("df_age_O.rds") %>% mutate(cohort = "Ottawa Trial") %>% 
  filter(item %in% df_shared$scent)
df_fine_P <- readRDS("df_fine_P.rds") %>% filter() %>% mutate(cohort = "PROBE") %>% 
  filter(item %in% df_shared$scent)
df_age_P <- readRDS("df_age_P.rds") %>% mutate(cohort = "PROBE") %>% 
  filter(item %in% df_shared$scent)
