...
# key
key   <- unlist(unname(read.csv("UPSIT_key.csv", header = FALSE)))
...
# Item labels
optlab  <- unname(read.csv("UPSIT_option.csv", header = FALSE))
optionLab <- list()
for (i in 1:n)
{
  if (noption[i] == 4) {
...
rankings <- read.csv("Published rankings.csv", header = TRUE)
...
# Save the rankings
write.csv(rank_comp, "UPSIT_rankings.csv", row.names = FALSE)
```
...
# add the shared ones
df_shared <- read.csv("scent_shared.csv", header = TRUE)
type_vec_Shared <- c()
indList_Shared<- list()
...
saveRDS(df_long_M, "df_sex_O_M.rds")
```
...
saveRDS(df_long_F, "df_sex_O_F.rds")
```
...
saveRDS(df_long_M, "df_sex_P_M.rds")
```
...
saveRDS(df_long_F, "df_sex_P_F.rds")
```
...
saveRDS(df_fine, "df_fine_O.rds")
saveRDS(df_long_Age, "df_age_O.rds")
```
...
saveRDS(df_fine, "df_fine_P.rds")
saveRDS(df_long_Age, "df_age_P.rds")
```
