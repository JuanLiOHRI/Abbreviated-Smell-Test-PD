...
# disease.duration is removed from the zenodo file as a further de-identification measure, 
# please uncomment below line if you get access to the original data file
...
# ===============================================
# If you download the further de-identification data from zenodo, 
# please run below data chunk
# ===============================================

# DeNoPa
df_D_tbl1 <- df_D %>% 
  select(group, 
         sex, age_cat, last_month, SST.ID, SST.ID.percentile, olfaction) 
names(df_D_tbl1)[c(2:ncol(df_D_tbl1))] <- c("Sex", "Age category", 
...
df_O_tbl1 <- df_O %>% 
  select(group, 
         sex, age_cat, UPSIT, UPSIT_percentile, olfaction) 
names(df_O_tbl1)[c(2:ncol(df_O_tbl1))] <- c("Sex", "Age category",
...
df_P_tbl1 <- df_P %>% 
  select(group, 
         sex, age_cat, UPSIT, UPSIT_percentile, olfaction) 
names(df_P_tbl1)[c(2:ncol(df_P_tbl1))] <- c("Sex", "Age category", 
                                            "UPSIT", "UPSIT_percentile", "Olfaction")
...
# ===============================================
# If you have access to the original data sets, 
# please uncomment and run below data chunk
# ===============================================
...
# DeNoPa
# zenodo data
fit <- glm(SST.ID ~ age_cat + sex + group, data = df_D)
# original data
...
# Ottawa Trial
# zenodo data
fit <- glm(UPSIT ~ age_cat + sex + group, data = df_O)
# original data
...
# PROBE
# zenodo data
fit <- glm(UPSIT ~ age_cat + sex + group, data = df_P)
# original data
