...
```{r}
df_D$ID.new <- as.factor(df_D$ID)
df_D <- df_D %>% mutate(ID.new = as.numeric(ID.new))
```

# reorder columns

```{r, echo=FALSE, message=FALSE, warning=FALSE}
df_D <- df_D %>% select(-c(ID, month, group.final, PD.NMS.02, leave.Visit))
names(df_D)[names(df_D) == "ID.new"] <- "ID"
